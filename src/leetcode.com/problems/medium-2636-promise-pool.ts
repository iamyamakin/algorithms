...
export async function promisePool(fns: (() => Promise<unknown>)[], limit: number): Promise<unknown> {
    const next: () => Promise<unknown> | undefined = () => fns[limit++]?.().then(next);

    return await Promise.all(fns.slice(0, Math.max(limit, 1)).map(fn => fn().then(next)));
}
