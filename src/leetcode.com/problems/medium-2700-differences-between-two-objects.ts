...
export function objDiff(one: object, two: object): object {
    if (one === two) {
        return {};
    }
    ...
    for (const key of Object.keys(one)) {
        ...
        const diff = objDiff(one[key as keyof typeof one], two[key as keyof typeof two]);
